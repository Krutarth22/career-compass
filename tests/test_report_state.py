...
def test_write_report_creates_file_with_expected_filename(tmp_path):
    ...
    assert p.name == "report-2026-01-15_10-30-00-12345678-ml-ai-engineer.docx"


def test_write_report_creates_sidecar_meta_file(tmp_path):
    roadmaps_dir = tmp_path / "roadmaps"
    frontmatter = _sample_frontmatter()
    ...
    meta_path = Path(path).with_name(Path(path).stem + ".meta.yaml")
    assert meta_path.exists()
    loaded = yaml.safe_load(meta_path.read_text())
    assert loaded["report_id"] == frontmatter["report_id"]
    assert loaded["target_state"] == frontmatter["target_state"]


def test_write_report_body_is_rendered_into_the_docx(tmp_path):
    roadmaps_dir = tmp_path / "roadmaps"
    frontmatter = _sample_frontmatter()

    path = report_state.write_report(
        roadmaps_dir, frontmatter, "# Roadmap\nBody text"
    )
...
def test_read_report_raises_on_missing_sidecar(tmp_path):
    path = tmp_path / "report-2026-01-15_10-30-00-abcd1234-slug.docx"
    path.write_bytes(b"not a real docx, but the sidecar is what's missing")

    with pytest.raises(ValueError):
        report_state.read_report(path)


def test_read_report_raises_when_sidecar_is_not_a_mapping(tmp_path):
    path = tmp_path / "report-2026-01-15_10-30-00-abcd1234-slug.docx"
    path.write_bytes(b"")
    meta_path = tmp_path / "report-2026-01-15_10-30-00-abcd1234-slug.meta.yaml"
...
def test_get_last_report_degrades_to_none_on_corrupt_newest_report(tmp_path):
    ...
    # A corrupt report (no sidecar .meta.yaml) whose embedded timestamp
    # sorts newest.
    corrupt_path = roadmaps_dir / "report-2026-06-01_00-00-00-99999999-ml.docx"
    corrupt_path.write_text("this is not a real docx and has no sidecar")
...
def test_get_last_report_returns_none_when_only_file_is_corrupt(tmp_path):
    ...
    roadmaps_dir.mkdir()
    corrupt_path = roadmaps_dir / "report-2026-06-01_00-00-00-99999999-ml.docx"
    corrupt_path.write_text("no sidecar meta file here")
