# career-compass — Implementation Plan
...
- Python `>=3.11`, stdlib + `pyyaml` only (no other third-party deps in `scripts/`).
  `pytest` for tests.
- All runtime scripts live under `.claude/skills/career-compass/scripts/`. All tests live
  under `tests/` at repo root and import scripts via `sys.path` manipulation or a
  `conftest.py` that adds `.claude/skills/career-compass/scripts/` to `sys.path` — set this
...
- Timestamps: UTC, ISO 8601, always via `datetime.now(timezone.utc)` — never naive
  `datetime.now()`.
- Do not implement `.claude/skills/career-compass/SKILL.md`'s prose/orchestration logic
...
- `.gitignore` containing (at minimum): `profile.yaml`, `roadmaps/report-*.md`,
  `tracker/career_compass_tracker.csv`, `__pycache__/`, `*.pyc`, `.pytest_cache/`,
...
- `pyproject.toml` declaring `pyyaml` and `pytest` as dependencies, Python
  `>=3.11`, project name `career-compass`.
...
- `roadmaps/.gitkeep` (empty file, so the directory exists in git despite its contents
  being ignored).
- `tracker/career_compass_tracker.csv.example` with header row:
...
- `tests/conftest.py` that inserts
  `<repo-root>/.claude/skills/career-compass/scripts` at the front of `sys.path` (compute
...
- Empty placeholder dirs (via `.gitkeep` or the module files that later tasks will
  add): `.claude/skills/career-compass/scripts/`, `.claude/skills/career-compass/reference/`,
  `.claude/skills/career-compass/templates/ai-ml-engineer/`, `.claude/skills/find-courses/`.

**Verify:** `git status` shows the expected files; `.gitignore` correctly matches
`roadmaps/report-2026-01-01-test.md` and `tracker/career_compass_tracker.csv` (spot-check
...
All under `.claude/skills/career-compass/reference/`. These are read by both SKILL.md (as
...
**Verify:** `python -c "import yaml; yaml.safe_load(open('.claude/skills/career-compass/reference/skill-taxonomy.yaml'))"` (or wherever you put the YAML) parses without error, likewise for the track-aliases YAML.

---

## Task 3: State I/O modules — `profile_io.py`, `tracker_io.py`, `report_state.py`

All three live in `.claude/skills/career-compass/scripts/`. Write full unit tests in
...
5. Assign `tier` — **this function does not compute tiering** (that's the LLM's
   ...
   on each `requirement` dict (i.e. Step 4 of `/career-compass` passes in requirements that
...
`.claude/skills/career-compass/templates/TEMPLATE.md` — the schema doc (frontmatter
fields + required body sections, as documented in Task 5's checks), meant for human
contributors, not parsed by code.

`.claude/skills/career-compass/templates/ai-ml-engineer/` — 7 templates. Two are fully
...
`python .claude/skills/career-compass/scripts/lint_templates.py` (or equivalent invocation
...
argument, `$1` = second. `/career-compass "<current-state>" "<target-state>"` → `$0` =
current-state, `$1` = target-state. `/career-compass confirm "<skill>" "<evidence>"` →
...
(`profile.yaml`, `tracker/career_compass_tracker.csv`, `roadmaps/`) resolve relative to
...
line if `profile.yaml` is available (resolve it the same way `/career-compass` does — via
`git rev-parse --show-toplevel`). Print to terminal only — never writes a file. State
explicitly at the top of this SKILL.md that `/career-compass`'s Step 7 invokes this exact
...
- `README.md`: what CareerCompass is, install (clone the repo — this is v1's explicitly
  repo-scoped packaging model, same convention as career-ops/ai-job-search, not a
  drop-in-anywhere skill), quickstart (`/career-compass "<current>" "<target>"`,
  `/find-courses <skill>`, `/career-compass confirm "<skill>"`), and an explicit **privacy
  note**: `profile.yaml`, `roadmaps/report-*.md`, and `tracker/career_compass_tracker.csv`
