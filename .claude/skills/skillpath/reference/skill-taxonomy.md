...
The skill taxonomy is the canonical registry of skill IDs that skillpath uses
...
1. **Template `skill_tags` / `skill_prerequisites` (hard requirement).**
   ...
   `.claude/skills/skillpath/templates/` **must** resolve to an `id` that
...
2. **Live research output (soft, provisional).** When skillpath resolves
...
This split exists so that the templates skillpath ships with are always
