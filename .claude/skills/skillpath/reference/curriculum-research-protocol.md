...
keep that judgment consistent across runs, the same way `skillpath`'s own
...
counts on its own" rule `skillpath/reference/research-protocol.md` applies
...
This mirrors `skillpath`'s own rule against inferring a skill from a bare
job-posting title.
