...
(`/skillpath career-suggestions "<degree>" ["<interests-or-context>"]` in
Claude Code, `$skillpath career-suggestions "<degree>" [...]` in Codex).
...
2. One closing line offering the handoff into the main flow, naming the
   ...
   /skillpath roadmap "recent <degree> graduate" "<chosen path>"
   ```

   (`$skillpath roadmap ...` in Codex.) Use a reasonable current-state
