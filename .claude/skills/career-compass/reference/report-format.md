...
8. **Next Steps** -- a short, concrete closing list: e.g. run
   `/career-compass record-evidence "<skill>" "<evidence>"` in Claude Code or
   `$career-compass record-evidence "<skill>" "<evidence>"` in Codex after finishing a
   course or project to record it, and re-run `/career-compass roadmap` again in N weeks (tie N
