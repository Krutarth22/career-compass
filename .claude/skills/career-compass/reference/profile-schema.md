...
`profile.yaml` holds a user's CareerCompass profile: current background, target
role, constraints, and preferences. It lives at the repo root (path resolved
the same way everywhere career-compass touches it — see the merge rule below) and
...
- `skill` is **free text the user typed** ("CAD Design", "Python", "Excel")
  ...
  `skill_id` at run time**, not stored resolved in the file: career-compass runs
...
career-compass can be invoked with CLI arguments that supply some or all profile
