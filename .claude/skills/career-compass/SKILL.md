---
name: career-compass
description: Guide a college-to-career journey through prerequisite-aware course planning, career suggestions, evidence-backed roadmaps, focused learning resources, and progress tracking. Routes to college-plan, career-suggestions, roadmap, find-courses, and record-evidence. Invoke explicitly with `/career-compass` in Claude Code or `$career-compass` in Codex; it writes profile/tracker/roadmap files, so it never runs on its own.
...
This is the single, explicit-invocation-only entry point for all CareerCompass
functionality. In Claude Code, invoke it with `/career-compass <command> ...`;
in Codex, invoke it with `$career-compass <command> ...`. Claude's
`disable-model-invocation` frontmatter and Codex's `agents/openai.yaml`
both enforce that policy. It writes to `profile.yaml`,
`tracker/career_compass_tracker.csv`, and `roadmaps/*.docx`, all of which are
personal, gitignored data.

Five commands form one learner journey, each keyed by a mandatory first word
(`$0`) rather than a positional guess. The usual progression is college
planning, career exploration, roadmap creation, focused learning, and evidence
capture. Users may still enter at whichever stage matches their current need.
Requiring the command word removes ambiguity and lets a bare `/career-compass`
print help instead of silently assuming intent:
...
it's a mode under `career-compass`, it inherits this skill's
explicit-invocation-only policy -- natural-language requests no longer
auto-trigger a course search; only `/career-compass find-courses <skill>` (or
`$career-compass find-courses <skill>` in Codex) does. This was a deliberate
...
SKILL_DIR="${CLAUDE_PLUGIN_ROOT:-${CLAUDE_SKILL_DIR:-${PROJECT_ROOT}/.agents/skills/career-compass}}"
```
...
entry. Codex discovers the repo-scoped symlink at `.agents/skills/career-compass`;
...
**Runtime state** (`profile.yaml`, `tracker/career_compass_tracker.csv`,
...
- **Bare invocation (help):** `/career-compass` / `$career-compass` with no
  arguments at all.
- **`roadmap`:** `/career-compass roadmap "<current-state>" "<target-state>"`
  in Claude Code, or `$career-compass roadmap "<current-state>" "<target-state>"`
  in Codex -- builds/updates the full career roadmap (`modes/roadmap.md`).
  Both state arguments are optional (see below).
- **`college-plan`:** `/career-compass college-plan "<major>" "<target_role>"
  ["<target_level>"]`.
- **`find-courses`:** `/career-compass find-courses <skill>`.
- **`career-suggestions`:** `/career-compass career-suggestions "<degree>"
  ["<interests-or-context>"]` -- explores career paths for a degree/major.
- **`record-evidence`:** `/career-compass record-evidence "<skill>" "<evidence>"`
  -- records evidence that closes a tracked gap.

Claude Code exposes positional arguments as `$0`, `$1`, `$2`, `$3`. Codex
does not; parse the text following the `$career-compass` mention into the
...
bare `/career-compass` show help instead of silently guessing what the user
wants.
...
```text
career-compass -- career roadmap and course-planning commands

  /career-compass roadmap ["<current-state>" "<target-state>"]
  ...
  /career-compass college-plan "<major>" "<target-role>" ["<target-level>"]
      Sequence college coursework for a major toward a target career.

  /career-compass find-courses <skill>
      Search for 2-3 current learning resources for one skill.

  /career-compass career-suggestions "<degree>" ["<interests-or-context>"]
  ...
  /career-compass record-evidence "<skill>" "<evidence>"
      Record evidence that closes a tracked skill gap.
```

(Substitute `$career-compass` for `/career-compass` when printing this under Codex.)
